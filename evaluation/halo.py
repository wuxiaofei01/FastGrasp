...
def intersection_eval_2(obj_faces,obj_verts, hand_faces , hand_verts, res=0.005, scale=1., trans=None, visualize_flag=False, visualize_file='output.off'):
    '''Calculate intersection depth and volumn of the two inputs meshes.
    args:
        mesh1, mesh2 (Trimesh.trimesh): input meshes
        res (float): voxel resolustion in meter(m)
    ...
    hand_verts = hand_verts.squeeze(dim=0)
    hand_verts = hand_verts.numpy()
    hand_faces = hand_faces.numpy()

    obj_faces = obj_faces.squeeze(dim = 0)
    obj_faces = obj_faces.numpy()
    obj_verts = obj_verts.squeeze(dim = 0)
    ...
    S, I, C = igl.signed_distance(obj_verts + 1e-10, hand_verts, hand_faces, return_normals=False)
    ...
    # Get bounding box for each mesh:
    min_corner0 = np.array([obj_verts[:,0].min(), obj_verts[:,1].min(), obj_verts[:,2].min()])
    max_corner0 = np.array([obj_verts[:,0].max(), obj_verts[:,1].max(), obj_verts[:,2].max()])

    min_corner1 = np.array([hand_verts[:,0].min(), hand_verts[:,1].min(), hand_verts[:,2].min()])
    max_corner1 = np.array([hand_verts[:,0].max(), hand_verts[:,1].max(), hand_verts[:,2].max()])
    ...
    S, I, C = igl.signed_distance(xyz, obj_verts, obj_faces, return_normals=False)
    ...
    S, I, C = igl.signed_distance(inside_samples, hand_verts, hand_faces, return_normals=False)
