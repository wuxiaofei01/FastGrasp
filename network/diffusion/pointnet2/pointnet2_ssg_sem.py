...
class PointNet2SemSegSSG(nn.Module):
    ...
    def build_SA_model(self, npoint, radius, nsample, feature_dim, mlp_depth, in_fea_dim,
        ...
        # pdb.set_trace()
        for i in range(len(npoint)): # [1024, 256, 64, 16]
    ...
    def build_FP_model(self, decoder_feature_dim, decoder_mlp_depth, feature_dim, in_fea_dim, include_t,
        ...
        for i in range(len(decoder_feature_dim)-1):
            ...
            if use_knn_FP:
                ...
                mlp1_spec = [decoder_feature_dim[i+1]] + \
                    [decoder_feature_dim[i]] * decoder_mlp_depth
                mlp2_spec = [decoder_feature_dim[i] + skip_feature_dim] + \
                    [decoder_feature_dim[i]] * decoder_mlp_depth
    ...
    def _build_model(self):
        ...
        if self.hparams["include_class_condition"]:
            self.class_emb = nn.Embedding(
                self.hparams["num_class"], self.hparams["class_condition_dim"])
        ...
        self.FP_modules = self.build_FP_model(decoder_feature_dim, decoder_mlp_depth, feature_dim, in_fea_dim,
                                              self.hparams['include_t'], self.hparams["include_class_condition"], radius=radius,
                                              nsample=nsample, use_knn_FP=self.use_knn_FP, K=self.K)
        ...
        if self.hparams["bn_first"]:
            self.fc_lyaer = nn.Sequential(
                nn.ReLU(True),
                nn.Conv1d(last_conv_in_dim,
                          self.hparams['out_dim'], kernel_size=1),
            )
        else:
            self.fc_lyaer = nn.Sequential(
                nn.Conv1d(last_conv_in_dim, 128, kernel_size=1,
        ...
        self.out_layer = nn.Linear(2048, 256)
    def forward(self, pointcloud, ts=None, label=None):
        ...
        if self.use_knn_FP:
            ...
            intermediate = torch.cat(
                [l_features[0], xyz.transpose(1, 2)], dim=1)
        else:
        ...
        with torch.no_grad():
            ...
            for i in range(len(SA_module)):
                for k in range(num_groupers_per_layer):
                    neigh_stats[k].append(
                        SA_module[i].groupers[k].neighbor_stats)
                    neigh_quantile[k].append(
                        SA_module[i].groupers[k].neighbor_num_quantile)

            for k in range(num_groupers_per_layer):
                neigh_stats[k] = torch.stack(neigh_stats[k], dim=0)
                neigh_quantile[k] = torch.stack(neigh_quantile[k], dim=0)
        for k in range(num_groupers_per_layer):
            print('%s: neighbor number (min, mean, max) in the %d-th grouper' %
                  (module_name, k))
            print(neigh_stats[k])
            print('%s: neighbor quantile (0-0.1-1) in the %d-th grouper' %
                  (module_name, k))
        ...
        if FP_module[0].include_grouper:
            with torch.no_grad():
                ...
                for i in range(len(FP_module)):
                    ...
                    neigh_quantile.append(
                        FP_module[i].grouper.neighbor_num_quantile)
    ...
    def report_neighbor_stats(self):
        if not self.record_neighbor_stats:
            ...
            return
        self.report_SA_module_neighbor_stats(
            self.SA_modules, module_name='Input cloud SA_module')
...
if __name__ == '__main__':
    # print(0)
    with open("/public/home/v-wuxf/CVPR/GraspTTA/affordance-CVAE/config/diffusion_cvae.json", 'r') as f:
        param = json.load(f)
    ...
    print(cloud.shape)
    cloud.requires_grad = True  # [64 , 2048 , 3]
    ts = torch.randint(10, (B,)).to(device)  # [64]  int
    label = torch.rand(B, 1024).to(device)
    out = pnet_sem(cloud, ts=ts, label=label)  # [64 , 2048 , 3]
    loss = out.mean()
    loss.backward()
    print("yes!!")
