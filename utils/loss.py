...
def kl_div_normal(output, target_mean=0.0, target_std=1.0):
    """
    计算96维向量输出与标准正态分布 N(0, 1) 之间的KL散度。
    
    参数:
    output (torch.Tensor) -- 形状为 [batch_size, 96] 的输出向量
    target_mean (float) -- 目标分布的均值，默认为0.0
    target_std (float) -- 目标分布的标准差，默认为1.0
    
    返回:
    torch.Tensor -- 批量样本的平均KL散度
    """
...
def CMap_loss3(obj_xyz, hand_xyz, cmap):
    ...
    if n_points == 0:
      cmap_loss = torch.tensor(0.0, dtype=torch.float32)
    else :
